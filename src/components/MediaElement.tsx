...
const MediaElement = (props: MediaElementProps) => {
    ...
    const handleDeleteClick = async () => {
        try {
            ...
            handleSuccess(snackbar, "Deleted")
        } catch (error: any) {
            handleError(snackbar, error)
        }
    }
        ...
        <div className="rounded overflow-hidden shadow-lg my-2 relative">
            {
                (mediaBase64 == null) ? (
                    <div className="grid place-items-center h-[200px]">
                        <div role="status">
                            <svg className="inline mr-2 w-8 h-8 text-gray-300 animate-spin dark:text-gray-600 fill-gray-900" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <>
                        <div className="absolute top-1 right-1 cursor-pointer">
                            <button onClick={handleDeleteClick}>
                                <DeleteIcon className="w-6 h-6" fill="#cc2121" />
                            </button>
                        </div>
